...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_invalid_boolean_type(self):
        """It should raise DataValidationError when deserialize with Invalid type for boolean [available]"""
        product = ProductFactory()
        product.id = None
        product.create()
        self.assertIsNotNone(product.id)
    ...
    def test_deserialize_missing_key(self):
        """It should raise DataValidationError when a key is missing"""
        product = ProductFactory()
        product.id = None
        product.create()
        self.assertIsNotNone(product.id)
        data = {  # Missing 'description', 'available'
            "id": product.id,
            "name": product.name,
            "price": str(product.price),
            "category": product.category.name
    ...
    def test_deserialize_invalid_category(self):
        """It should raise DataValidationError for an invalid category string"""
        product = ProductFactory()
        product.id = None
        product.create()
        self.assertIsNotNone(product.id)
    ...
    def test_find_by_price_str(self):
        """It should Find Products by Price as str"""
        products = ProductFactory.create_batch(10)
        for product in products:
            product.create()
        price = products[0].price
